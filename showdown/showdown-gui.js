...
var inputPane,previewPane,outputPane,syntaxPane,namePane;
...
function startGui() {
	...
	namePane = document.getElementById("name").value;
	...
	if (top.document.location.href.match(/\?blank=1$/))
		inputPane.value = "";
  if (location.href.match(/\?doc=/))  {
  	console.log(location.href.split('?doc=')[1])
  	var name = unescape(location.href.split('?doc=')[1]);
    inputPane.value = localStorage[name] === undefined ? "" : localStorage[name];
...
function convertText() {
	...
	// Do the conversion
	var namePane = document.getElementById('name').value;
	window.LS = localStorage;
	if (LS[namePane] !== undefined) {
		LS.setItem(namePane,text);
		text = LS[namePane];
	} else if (namePane !== ''){
		LS.setItem(namePane,text);
	}
